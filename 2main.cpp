...
string createsvg(OselinDevice *device, bool header, bool meas, float offset){
    /**
     * CHOSEN PARAEMTERS:
     * @param SVG WIDTH
     * @param SVG HEIGHT
    ...
    if (!oselin_init(device, carlength, carheight, ncars, nfloors, radius)){
        ...
        if (offset == 0) offset = (device->param.svgwidth-device->length)/2;
        return oselin_to_svg(device, header, meas, offset);
    }
...
int main(int argc, string argv) {
    ...
    OselinDevice *test = new OselinDevice;
    string svg;
    char a = menu();
    while (a != 'e'){
        switch (a)
        ...
        {            
            ...
            cin >> temp;
            loadsvg(device, temp);
            svg = oselin_to_svg(device, true, false, 0);
            break;
        }
        case 'c':       //CREATING AN SVG
            svg = createsvg(device, true, false, 0);
        ...
        case 'k':       //CREATING AN SVG
            svg = createsvg(device, false, false, 0);
            svg = createsvg(test, true, false, device->abslength) + svg;
